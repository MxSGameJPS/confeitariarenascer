...
function commandCode(command) {
  const physical = String(command?.command_label || "").trim().toUpperCase();
  return /^C[0-9]+$/.test(physical) ? physical : formatCommandCode(command.order_number);
}
...
export default function CounterCommandPanel({ surface = "staff" }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [salesBody, tablesBody] = await Promise.all([
        salesResponse.json(),
        tablesResponse.json(),
      ]);
  ...
  async function openCommand(event) {
    ...
    if (normalized.length < 2) {
      setMessage("Informe o nome ou uma identificação para o cliente.");
      return;
    }
    ...
    try {
      ...
      setMessage(`Comanda ${commandCode(body.data)} aberta. Agora você já pode lançar os produtos do balcão.`);
      await load();
    } catch (error) {
  ...
  async function addQuickItem() {
    ...
    if (!command || !product || !Number.isInteger(quantity) || quantity < 1 || quantity > 99) {
      setMessage("Selecione a comanda, localize o produto e informe uma quantidade válida.");
      return;
    }

    const item = { productId: product.id, quantity };
    if (product.pricing_mode === "variable") {
      const value = window.prompt(`Informe o valor calculado após pesar “${product.name}”:`, "");
    ...
    try {
      ...
      setItemProduct(null);
      setItemQuantity(1);
      setMessage(`${quantity}× ${product.name} adicionado à comanda ${commandCode(command)}.`);
      await load();
    } catch (error) {
      setMessage(error.message);
    } finally {
  ...
  async function linkTable(command) {
    ...
    if (!tableId) {
      setMessage("Selecione a mesa antes de vincular a comanda.");
      return;
    }
    ...
    try {
      ...
      setMessage(`Comanda ${commandCode(command)} vinculada à Mesa ${body.data.table_number}.`);
      await load();
    } catch (error) {
  ...
  return (
    <section className={styles.panel}>
      <div className={styles.heading}>
        <div>
          <span>BALCÃO</span>
          <h2>Comandas sem mesa</h2>
          <p>Abra a comanda no primeiro pedido e vincule a mesa somente quando o cliente sentar.</p>
        </div>
      ...
      <form className={styles.openForm} onSubmit={openCommand}>
        <label>
          <span>Nome ou identificação do cliente</span>
          <input
            value={label}
        ...
        <button disabled={busy || label.trim().length < 2}>
          {busy ? "Aguarde..." : "+ Nova comanda"}
        </button>
      </form>
      ...
      {counterCommands.length > 0 && (
        <div className={styles.quickAdd}>
          ...
          <select value={itemCommandId} onChange={(event) => setItemCommandId(event.target.value)}>
            ...
            {counterCommands.map((command) => (
              <option key={command.id} value={command.id}>
                {commandCode(command)} · {command.table ? `Mesa ${command.table.table_number}` : command.command_label || "Balcão"}
              </option>
            ))}
          </select>
          ...
          {itemProduct && (
            <div className={styles.selectedProduct}>
              <div>
                ...
                <strong>{itemProduct.name}</strong>
                <span>
                  {itemProduct.reference ? `Ref. ${itemProduct.reference} · ` : ""}
                  {itemProduct.gemaster_code ? `GeMaster ${itemProduct.gemaster_code} · ` : ""}
                  {itemProduct.pricing_mode === "variable" ? "por peso" : money.format(Number(itemProduct.price || 0))}
                </span>
              </div>
              <button type="button" className={styles.changeProduct} onClick={() => setItemProduct(null)}>
                Trocar
              </button>
            </div>
          )}
          ...
          <button
            type="button"
            disabled={busy || !itemCommandId || !itemProduct}
      ...
      <div className={styles.cards}>
        {counterCommands.length === 0 && (
          <p className={styles.empty}>Nenhuma comanda aberta está aguardando mesa.</p>
        )}
        {counterCommands.map((command) => (
          <article key={command.id}>
            <div className={styles.commandInfo}>
              <span>COMANDA {commandCode(command)}</span>
              <strong>{command.command_label || "Cliente do balcão"}</strong>
              <small>
                {command.items?.filter((item) => item.status === "ativo").length || 0} item(ns) · {money.format(Number(command.total || 0))}
              </small>
            </div>
            <div className={styles.linkArea}>
              <select
                value={tableSelection[command.id] || ""}
                onChange={(event) => selectTable(command.id, event.target.value)}
                aria-label={`Mesa para a comanda ${commandCode(command)}`}
              >
                <option value="">Vincular a uma mesa...</option>
                {tables.map((table) => (
                  <option key={table.id} value={table.id}>
                    Mesa {table.table_number} · {table.occupancy_status === "livre" ? "livre" : "em atendimento"}
                  </option>
                ))}
              </select>
              <button
                type="button"
                disabled={busy || !tableSelection[command.id]}
                onClick={() => linkTable(command)}
              >
                Vincular
              </button>
            </div>
      ...
      <small className={styles.hint}>
        Comanda física mantém o mesmo código no fluxo inteiro: C105 continua C105 no Bridge e no GeMaster.
      </small>
    </section>
  );
}
