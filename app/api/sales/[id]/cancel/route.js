...
export async function POST(request, { params }) {
  try {
    const actor = await requirePermissionSession(PERMISSIONS.SALES_CANCEL, surface(request));
    const { id } = await params;
    return await cancelSaleController(validateSaleId(id), validateCancellation(await request.json()), actor);
