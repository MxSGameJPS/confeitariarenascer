-- Corrige a importação do catálogo GeMaster quando códigos distintos possuem o mesmo nome.
-- Um produto Renascer já mapeado para o GeMaster não pode ser reutilizado por outro código externo.
...
begin
  ...
  loop
    ...
    -- Só reaproveita um produto existente quando ele ainda não possui vínculo GeMaster.
    -- Isso evita que dois CodigoProduto diferentes sejam associados ao mesmo UUID apenas
    -- porque o GeMaster possui nomes duplicados no catálogo.
    select count(*)
      into v_match_count
...
comment on function public.import_gemaster_products(jsonb, uuid) is
  'Importa catálogo GeMaster de forma idempotente usando external_code como identidade externa; nomes repetidos com códigos distintos permanecem como produtos distintos.';
